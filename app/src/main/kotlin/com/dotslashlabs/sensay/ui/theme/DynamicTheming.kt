...
class DominantColorState(
    ...
    suspend fun updateColorsFromImageUrl(uri: Uri): Pair<Color, Color> {
        val result = calculateDominantColor(uri)
        color = result?.color ?: defaultColor
        onColor = result?.onColor ?: defaultOnColor
        return Pair(color, onColor)
    }

    private suspend fun calculateDominantColor(uri: Uri): DominantColors? {
        ...
        return calculateSwatchesInImage(context, uri)
            ...
            ?.also { result -> cache?.put(uri.toString(), result) }
    }
...
/**
 * Fetches the given [imageUri] with [Coil], then uses [Palette] to calculate the dominant color.
 */
